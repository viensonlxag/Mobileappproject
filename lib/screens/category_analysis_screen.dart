...
class _CategoryAnalysisScreenState extends State<CategoryAnalysisScreen> {
  ...
  double _maxYTrend = 1; // Giá trị max cho trục Y của biểu đồ đường, ít nhất là 1 để tránh lỗi
  ...
  void initState() {
    super.initState();
    // Dữ liệu sẽ được tính toán lần đầu khi Consumer build và _updateAllAnalysis được gọi
  }
  ...
  void _prepareCategoryTrendData(AppProvider appProvider) {
    ...
    while (!loopDate.isAfter(selectedDateRange.end)) {
      ...
      if (monthsInRange.length > 36) break; // Giới hạn 3 năm dữ liệu xu hướng
    }
    ...
    for (int i = 0; i < topCategoriesForTrend.length; i++) {
      ...
      for (int monthIndex = 0; monthIndex < monthsInRange.length; monthIndex++) {
        ...
        DateTime monthEnd = DateTime(monthStart.year, monthStart.month + 1, 0, 23, 59, 59, 999);
        ...
        final spotY = spendingInMonthForCategory / 1000000; // Chia cho 1 triệu
      ...
      if (spots.isNotEmpty) {
        lineBars.add(
            LineChartBarData(
              spots: spots,
              isCurved: true,
              color: CategoryHelper.getCategoryColor(category, 'Chi tiêu'),
              barWidth: 3,
              isStrokeCapRound: true,
              dotData: const FlDotData(show: true, getDotPainter: _customDotPainter), // Hiển thị điểm rõ hơn
              belowBarData: BarAreaData(show: true, color: CategoryHelper.getCategoryColor(category, 'Chi tiêu').withOpacity(0.1)),
            )
        );
      }
    }
    if(mounted){
      if (!listEquals(_trendMonthLabels, newTrendMonthLabels) || !listEquals(_trendLineBarsData, lineBars) || _maxYTrend != (currentMaxY == 0 ? 1 : currentMaxY * 1.2)) {
        setState(() {
          _trendMonthLabels = newTrendMonthLabels;
          _trendLineBarsData = lineBars;
          _maxYTrend = currentMaxY == 0 ? 1 : currentMaxY * 1.2; // Thêm 20% padding cho trục Y
        });
      }
    }
  }
  ...
  bool listEquals<T>(List<T>? a, List<T>? b) {
    ...
    for (int i = 0; i < a.length; i++) {
      // For FlSpot, need to compare x and y. For LineChartBarData, it's more complex.
      // This simple comparison might not be enough for deep equality of complex objects.
              ...
              children: [
                ...
                if (appProvider.isLoading && _trendLineBarsData.isEmpty)
                  ...
                  Card(
                    ...
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                    child: Padding(
                      padding: const EdgeInsets.fromLTRB(8, 24, 16, 12), // Giảm padding trái
                      child: SizedBox(
                        height: 250,
                        child: LineChart(
  ...
  LineChartData _buildLineChartData(ThemeData theme) {
    return LineChartData(
      gridData: FlGridData(
        ...
        horizontalInterval: _maxYTrend / 5 > 0 ? _maxYTrend / 5 : 1, // Điều chỉnh interval cho phù hợp
        verticalInterval: 1,
      ...
      titlesData: FlTitlesData(
          ...
          sideTitles: SideTitles(
            ...
            getTitlesWidget: (double value, TitleMeta meta) {
              ...
              if (index >= 0 && index < _trendMonthLabels.length) {
                if (_trendMonthLabels.length <= 6 || index == 0 || index == _trendMonthLabels.length -1 || index % ((_trendMonthLabels.length / 4).ceilToDouble()).toInt() == 0 ) {
        ...
        leftTitles: AxisTitles(
          sideTitles: SideTitles(
            showTitles: true,
            reservedSize: 42, // Tăng khoảng trống cho nhãn
            getTitlesWidget: (double value, TitleMeta meta) {
              if (value == meta.max || value == meta.min && value !=0) return const Text('');
              return Padding(
                padding: const EdgeInsets.only(left: 4.0),
                child: Text(NumberFormat.compact(locale: 'vi_VN').format(value * 1000000), // Sử dụng NumberFormat.compact
                    style: theme.textTheme.bodySmall?.copyWith(fontSize: 9.5), textAlign: TextAlign.left),
              );
            },
            interval: _maxYTrend / 5 > 0 ? _maxYTrend / 5 : 1, // Điều chỉnh interval
          ),
        ),
      ),
      ...
      minY: 0,
      maxY: _maxYTrend, // Sử dụng _maxYTrend đã tính toán
        ...
        touchTooltipData: LineTouchTooltipData(
          ...
          getTooltipItems: (List<LineBarSpot> touchedBarSpots) {
            return touchedBarSpots.map((barSpot) {
              ...
              // Lấy tên danh mục từ _trendLineBarsData (cần cách để lưu trữ hoặc ánh xạ)
              // Tạm thời dùng index để lấy từ _topSpendingCategories (nếu số lượng khớp)
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            title: Text(entry.key, style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
            subtitle: Text('${percentage.toStringAsFixed(1)}% tổng chi tiêu', style: theme.textTheme.bodySmall?.copyWith(color: Colors.grey[600])),
            trailing: Text(currencyFormatter.format(entry.value), style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.bold, color: theme.colorScheme.primary)),
          ),
        );
      },
  ...
  List<PieChartSectionData> _buildPieChartSections(List<MapEntry<String, double>> sortedCategories, ThemeData theme) {
    return List.generate(sortedCategories.length, (i) {
      ...
      final radius = isTouched ? 65.0 : 55.0; // Tăng kích thước biểu đồ tròn
      ...
      return PieChartSectionData(
        ...
        value: entry.value,
        title: percentage > 3 ? '${percentage.toStringAsFixed(0)}%' : '', // Chỉ hiển thị % nếu đủ lớn
        radius: radius,
        ...
        return Padding(
          ...
          child: Row(
            children: [
              ...
              SizedBox(
                width: 80, // Cho cột số tiền có độ rộng cố định để căn chỉnh
                child: Text(
